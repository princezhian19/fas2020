...
$bb = 15;
  ...
  $query1 = "SELECT RO_OT_OB, PERDIEM + RECEIPT AS 'a' ,tbltravel_claim_info.`ID` as 'dID', `TC_ID`, `RO`, `DATE`, `PLACE`, `ARRIVAL`, `DEPARTURE`, `MOT`, `TRANSPORTATION`, `PERDIEM`, `RECEIPT`,`OTHERS`, `TOTAL_AMOUNT`,
  ...
  // echo $query1;
    if(mysqli_num_rows($result1) > 0)
    {
      ...
      {
        // echo $row1['RO_OT_OB'];
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$bb,$row1['RO_OT_OB']);     

        $aa = $objPHPExcel->setActiveSheetIndex(0)->getHighestRow();

        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$row,$row1['DATE']);     
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$row,$row1['PLACE']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$row,date('g:i A',strtotime($row1['ARRIVAL'])));
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$row,date('g:i A',strtotime($row1['DEPARTURE'])));
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$row,$row1['MOT']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$row,sprintf("%.2f",$row1['TRANSPORTATION']));
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$row,$perdiem);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$row,$row1['OTHERS']);
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J'.$row,sprintf("%.2f",$row1['TOTAL_AMOUNT']));
