...
$comm = mysqli_query($conn,"SELECT cb.office,cb.comments,pmo.pmo_title,cb.date_stamp FROM comments_burs cb LEFT JOIN pmo on pmo.id = cb.office WHERE cb.burs_id = '$id' ORDER BY cb.id DESC");

$comm2 = mysqli_query($conn,"SELECT * FROM comments_burs  WHERE burs_id = '$id' ORDER BY id DESC");
$row2 = mysqli_fetch_array($comm2);
$date_stamp = $row2['date_stamp'];
$burs_id = $row2['burs_id'];
$date = date('F d Y ', strtotime($date_stamp));
$time = date(' F d Y h:i a', strtotime($date_stamp));

if (isset($_POST['submit'])) {
  $comments1 = $_POST['comments'];
  $insert = mysqli_query($conn,"INSERT INTO comments_burs(office,comments,burs_id) VALUES(0,'$comments1','$burs_id1')");
  if ($insert) {
    $update = mysqli_query($conn,"UPDATE burs SET status = 3, date_return = now(),date_received = NULL WHERE id = '$id' ");
