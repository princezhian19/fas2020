...
function inetmainmenu_v2($parent,$level)
{
	...
	<?php
			...
			$query = "SELECT ACCESSLIST FROM tblemployeeinfo WHERE CODE='".$_SESSION['inet_credentials']['code']."'";
...
function IsthereAccess($section){
	...
	$query = "SELECT ACCESSLIST FROM tblemployeeinfo WHERE CODE='".$_SESSION['inet_credentials']['code']."' LIMIT 1";
...
function cntregacc()
{
	...
	$sqlCount = "SELECT count(EMP_N) as cnt FROM tblemployeeinfo WHERE tblemployeeinfo.REGION_C = ".$_SESSION['inet_credentials']["region"]." AND tblemployeeinfo.INVI = 'No' AND activated = 'No'	AND tblemployeeinfo.UNAME IS NOT NULL AND  tblemployeeinfo.PSWORD IS NOT NULL";						
