...
//$results = $conn->prepare("SELxCT  supplier_quote.id,supplier.supplier_title, po.po_no, rfq.purpose FROM selected_quote sq LEFT JOIN rfq ON rfq.id = sq.rfq_id LEFT JOIN supplier_quote ON supplier_quote.id = sq.supplier_quote_id LEFT JOIN supplier ON supplier_quote.supplier_id = supplier.id LEFT JOIN po ON po.id = sq.po_id  WHERE po_no LIKE '%".$q."%' LIMIT 1");
$results = $conn->prepare("SELECT * FROM disbursement   WHERE dv ='$q' LIMIT 1");
}
else
{
...
$q = $_POST["query"];
$results = $conn->prepare("SELECT * FROM disbursement  WHERE dv ='$q' LIMIT 1");
/* echo "SELECT * FROM disbursement  WHERE dv ='$q' LIMIT 1";
exit(); */
}
...
while($row = $results->fetch(PDO::FETCH_ASSOC))
{
    ...
    '<td style="text-align: center;" >' . $row['dv'] . '</td>' . 
    '<td hidden>' . $row['ors'] . '</td>' . 
    '<td hidden>' . $row['flag'] . '</td>' .
    '<td hidden>' . $row['amount'] . '</td>' .
    '<td hidden>' . $row['total'] . '</td>' .
    '<td hidden>' . $row['net'] . '</td>' .
    '</tr>';
