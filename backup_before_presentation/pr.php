...
if (isset($_POST['submit'])) {
  ...
  if (mysqli_num_rows($check)>0) {
    ...
    $insert_pr = mysqli_query($conn,"INSERT INTO pr(pr_no,pmo,purpose,pr_date) VALUES('$pr_no1','$pmo1','$purpose1','$pr_date1')");
    ...
    {  
     ...
     $update_minus = mysqli_query($conn,'UPDATE app_items SET qty_original = qty_original - '.$_POST['qty1'][$count].' WHERE pmo_id = '.$pmo1.' AND sn = "'.$snAi.'" ');

     echo 'UPDATE app_items SET qty_original = qty_original - '.$_POST['qty1'][$count].' WHERE pmo_id = '.$pmo1.' AND sn = "'.$snAi.'"';
   }
  ...
  if ($app_items == '') {
    ...
    if ($pmo == 'Please Select') {
      ...
      if ($app_items =="dummy") {
        ...
        if ($unit == 0 ) {
          echo ("<SCRIPT LANGUAGE='JavaScript'>
            window.alert('WARNING : Please Select Unit!');
            </SCRIPT>");
        }else{

         $insert_items = mysqli_query($conn,'INSERT INTO pr_approved(pr_no,items,description,unit,qty,abc) 
          VALUES("'.$_POST['pr_no'].'","'.$_POST['app_items'].'","'.$_POST['description'].'","'.$_POST['unit'].'","'.$_POST['qty'].'","'.$_POST['abc'].'")');
...
<head>
  ...
  <script type="text/javascript">
    ...
    function showRow(row)
    {
      ...
      document.getElementById("abc").value = x[2].innerHTML;
    ...
    <form method="POST" >
      <div class="box-body">
        <div class="well">
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <?php if ($pr_no != ''): ?>
                 ...
                 <input readonly autocomplete = "off" value="<?php echo $getDate.'-'.'00'.$idGet?>" class="form-control" name="pr_no" type="text" id="pr_no" class="demoInputBox" onBlur="checkAvailability()"><span id="user-availability-status"></span>
                 <?php else:  ?>
             ...
             <div class="form-group">
                ...
                <?php else: ?>
                  ...
                  <?php endif ?>
                <!-- <input readonly = "off" value="<?php echo isset($_POST['pmo']) ? $_POST['pmo'] : '' ?>" class="form-control" name="pmo" type="text" id="pmo"> -->
                <?php endif ?>
            </div>
          ...
          <div class="col-md-6">
            ...
            <div class="form-group">
                  ...
                  <div class="input-group date">
                    ...
                    <?php if ($pr_date ==''): ?>
                      <input readonly type="date" class="form-control pull-right" name="pr_date" id="pr_date" value="<?php echo $now = date_create()->format('Y-m-d'); ?>">
                      <?php else: ?>
                        <input readonly type="date" class="form-control pull-right" name="pr_date" id="pr_date" value="<?php echo $now = date_create()->format('Y-m-d'); ?>">
                      <?php endif ?>
                    </div>
          ...
          <div class="item panel panel-info"><div class="panel-heading"> <p style = "color:red;font-size:16px;"> <b> NOTE : </b> Please do not include this characters <b>( ' and " ) </b> the system will not accept this characters.Applicable to all fields. </p> </div></div>
          <?php 
          if ($insert_items) {
            ...
            <div class=""><!-- widgetBody -->
              <div class="row">
                <div class="col-md-6" style="padding-left: 30px;padding-top:10px;">
                  <label>Item </label>
                  <input type="text" class="form-control" name="app" id="app_items" placeholder="Search Code" class="" />
                ...
                <div class="col-md-6">
                  ...
                  <div class="form-group">
                    ...
                    <input class="form-control" type="text" id="abc"  name="abc">
                  </div>
        ...
        <div class="panel-body container-items">
         ...
         <tr>
          ...
          $sql_items = $conn->query("SELECT pa.id,pa.qty,pa.items,pa.app_id,pa.pr_no,pa.description,pa.unit,pa.abc,a.procurement FROM pr_approved pa left join app a on a.id = pa.items  WHERE pa.pr_no = '$pr_no' ");
