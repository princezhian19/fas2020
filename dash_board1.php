  ...
  <div class="col-md-3">
    <div class="box" style="outline: lightgray solid 10px;">
      ...
      <div class="box-header">
        <?php 
        $conn=mysqli_connect("localhost","fascalab_2020","w]zYV6X9{*BN","fascalab_2020");
        $BDAY = mysqli_query($conn,"SELECT FIRST_M,MIDDLE_M,LAST_M,BIRTH_D,PROFILE FROM tblemployeinfo WHERE MONTH(BIRTH_D) =MONTH(NOW()) LIMIT 5");
          ...
          <div class="box-body">
            <div class="row">
              <div class="col-md-5">
                ...
                <br>
                <?php 
                $select_gender = mysqli_query($conn,"SELECT count(*) as female FROM tblemployeinfo WHERE SEX_C = 'Female' AND ACTIVATED = 'Yes' ");
                $rowG = mysqli_fetch_array($select_gender);
                $female = $rowG['female'];

                $select_genderB = mysqli_query($conn,"SELECT count(*) as male FROM tblemployeinfo WHERE SEX_C = 'Male' AND ACTIVATED = 'Yes' ");
                $rowGB = mysqli_fetch_array($select_genderB);
                $male = $rowGB['male'];

                $reg = $female + $male;

                $select_gender2 = mysqli_query($conn,"SELECT count(*) as female FROM tblemployeinfo WHERE SEX_C = 'Female' AND ACTIVATED = 'No' ");
                $rowG2 = mysqli_fetch_array($select_gender2);
                $female2 = $rowG2['female'];

                $select_genderB2 = mysqli_query($conn,"SELECT count(*) as male FROM tblemployeinfo WHERE SEX_C = 'Male' AND ACTIVATED = 'No' ");
              ...
              <!-- /.col -->
              <?php 
              $count_region = mysqli_query($conn,"SELECT count(*) as region FROM tblemployeinfo WHERE OFFICE_STATION = 1");
              $cregion = mysqli_fetch_array($count_region);
              $region = $cregion['region'];
              ?>
              <?php 
              $count_batangas = mysqli_query($conn,"SELECT count(*) as batangas FROM tblemployeinfo WHERE DIVISION_C IN (19,28,29,30,44)");
              $cbatangas = mysqli_fetch_array($count_batangas);
              $batangas = $cbatangas['batangas'];
              ?>
              <?php 
              $count_cavite = mysqli_query($conn,"SELECT count(*) as cavite FROM tblemployeinfo WHERE DIVISION_C IN (20,34,35,36,45)");
              $ccavite = mysqli_fetch_array($count_cavite);
              $cavite = $ccavite['cavite'];
              ?>
              <?php 
              $count_laguna = mysqli_query($conn,"SELECT count(*) as laguna FROM tblemployeinfo WHERE DIVISION_C IN (21,40,41,42,47,51,52)");
              $claguna = mysqli_fetch_array($count_laguna);
              $laguna = $claguna['laguna'];
              ?> 
              <?php 
              $count_rizal = mysqli_query($conn,"SELECT count(*) as rizal FROM tblemployeinfo WHERE DIVISION_C IN (23,37,38,39,46,50,52)");
              $crizal = mysqli_fetch_array($count_rizal);
              $rizal = $crizal['rizal'];
              ?> 
              <?php 
              $count_quezon = mysqli_query($conn,"SELECT count(*) as quezon FROM tblemployeinfo WHERE DIVISION_C IN (22,31,32,33,48,49,53)");
              $cquezon = mysqli_fetch_array($count_quezon);
              $quezon = $cquezon['quezon'];
              ?> 
              <?php 
              $count_lucena = mysqli_query($conn,"SELECT count(*) as lucena FROM tblemployeinfo WHERE DIVISION_C IN (24)");
