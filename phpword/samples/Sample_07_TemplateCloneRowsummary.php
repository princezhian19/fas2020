...
left JOIN tblemployeeinfo c on a.added_by=c.EMP_N
left join tblpersonneldivision d on b.ROUTED_TO=d.DIVISION_N
where DATE = '".$date."' and a.ADDED_BY = ".$byme." and c.DIVISION_C = ".$division." ";
}
else{
$sql = "SELECT *,a.record_N as arecord  FROM `tblrecords` a 
left join tblrouting b  on a.RECORD_N=b.RECORD_N 
...
left join tblpersonneldivision d on b.ROUTED_TO=d.DIVISION_N
  left join tblrecordsources as e on e.SOURCE_N = a.SOURCE 
where DATE = '".$date."' and c.DIVISION_C = ".$division." ";
}
...
foreach ($query_data as $key) {
    ...
    $templateProcessor->setValue('rowpday#'.$i, $compare == $key['DATE_ROUTED'] ? '1' : $interval->d );
...
$templateProcessor->saveAs('results/SUMMARY_INCOMING_OUTGOING-COMMUNICATIONS.docx');
...
<script type="text/javascript">
    var delay = 100; 
setTimeout(function(){ window.location = "http://www.loop.calabarzon.dilg.gov.ph/phpword/samples/results/SUMMARY_INCOMING_OUTGOING-COMMUNICATIONS.docx"; }, delay);
</script>
