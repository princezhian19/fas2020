...
$results = $conn->prepare("SELECT * FROM saroobburs  WHERE burs LIKE '%".$q."%' LIMIT 1");
}
else
{
...
$q = $_POST["query"];
$results = $conn->prepare("SELECT sum(amount) as amount, payee, particular, datereleased ,burs,saronumber,ppa  FROM saroobburs  WHERE burs LIKE '".$q."%'  ");
