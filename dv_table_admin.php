      ...
      <table id="example1" class="table table-bordered-striped table-bordered" style="width:;background-color: white;">
        ...
        while ($row = mysqli_fetch_assoc($view_query)) {
          ...
          <tr>
            ...
            <?php if ($status == 2 || $status == 4 || $status == 5 || $status == 6): ?>
              <td><?php echo $date_received;?></td>
              <?php else: ?>
            <td><a class="btn btn-primary btn-xs" href='received_dv.php?id=<?php echo $id; ?>&stat=1' >Received</a> </a></td>
            <?php endif ?>
            <?php if ($status == 4 || $status == 5 || $status == 6 ): ?>
              <td><?php echo $date_return;?></td>
              <?php else: ?>
                <td>
             <a class="btn btn-danger btn-xs" href='ViewDVcomments.php?id=<?php echo $id; ?>&stat=2'>Return</a>
           </td>
            <?php endif ?>
            <?php if ($status == 4 || $status == 5 || $status == 1 ): ?>
              <td><?php echo $date_proccess;?></td>
              <?php else: ?>
                <td>
            <a class="btn btn-success btn-xs" href='CreateDisbursement.php?id=<?php echo $id; ?>&stat=1' >Proccess</a> 
          </td>

            <?php endif ?>
            <?php if ($status == 5 || $status == 1 || $status == 2): ?>
              <td><?php echo $date_release;?></td>
              <?php else: ?>
                <td>
            <a class="btn btn-success btn-xs" href='release_dv.php?id=<?php echo $id; ?>&stat=1' >Release</a> 
          </td>
            <?php endif ?>
