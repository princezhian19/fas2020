...
$results = $conn->prepare("SELECT * FROM saroob  WHERE ors LIKE '%".$q."%' and dvstatus=NULL LIMIT 1");
}
else
{
...
$q = $_POST["query"];
$results = $conn->prepare("SELECT sum(amount) as amount, payee, particular, datereleased,ors,saronumber,ppa  FROM saroob  WHERE ors = '$q' and dvstatus=''  ");

/* echo "SELECT sum(amount) as amount, payee, particular, datereleased,ors,saronumber,ppa  FROM saroob  WHERE ors LIKE '%".$q."%' ";
exit(); */
}
